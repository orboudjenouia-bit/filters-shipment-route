...
router.post(
  ...
  [
    check("password", "Password must be at least 10 Alphanumerical characters").isLength({ min: 10 })
  ],
...
router.put("/update-profile",
   ...
   ,[
    ...
    check("user.type", "type is REQUIRED and must be INDIVIDUAL or BUSINESS").exists({ checkNull: true}).isIn(["INDIVIDUAL", "BUSINESS"]),
    check("user.phone", "Phone must be exactly 10 digits").optional().matches(/^\d{10}$/),
    check("user.working_Time", "Working time format should be HH:MM-HH:MM").optional().matches(/^\d{1,2}:\d{2}-\d{1,2}:\d{2}$/),
