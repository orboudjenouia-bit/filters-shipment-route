...
router.post('/', 
  ...
  [
    ...
    check('date', 'Date must be in format YYYY-MM-DD').optional({ nullable: true }).matches(/^\d{4}-\d{2}-\d{2}$/),
    check('vehicle_plate', 'Vehicle plate number is required').notEmpty().isNumeric(),
    check('post_type', 'Post type must be REGION or ORIGIN_DESTINATION').optional().isIn(['REGION', 'ORIGIN_DESTINATION']),
    check('date_type', 'Date type must be DAY or INTERVAL').optional().isIn(['DAY', 'INTERVAL']),
    check('interval_start', 'Interval start must be YYYY-MM-DD').optional({ nullable: true }).matches(/^\d{4}-\d{2}-\d{2}$/),
    check('interval_end', 'Interval end must be YYYY-MM-DD').optional({ nullable: true }).matches(/^\d{4}-\d{2}-\d{2}$/),
  ],
...
router.patch('/',
  ...
  [
    ...
    check('date', 'Date must be YYYY-MM-DD or YYYY-MM-DD to YYYY-MM-DD').optional().matches(/^\d{4}-\d{2}-\d{2}(\s+to\s+\d{4}-\d{2}-\d{2})?$/),
    check('vehicle_plate', 'Vehicle plate must be numeric').optional().isNumeric(),
    check('post_type', 'Post type must be REGION or ORIGIN_DESTINATION').optional().isIn(['REGION', 'ORIGIN_DESTINATION']),
    check('date_type', 'Date type must be DAY or INTERVAL').optional().isIn(['DAY', 'INTERVAL']),
    check('interval_start', 'Interval start must be YYYY-MM-DD').optional().matches(/^\d{4}-\d{2}-\d{2}$/),
    check('interval_end', 'Interval end must be YYYY-MM-DD').optional().matches(/^\d{4}-\d{2}-\d{2}$/)
  ],
