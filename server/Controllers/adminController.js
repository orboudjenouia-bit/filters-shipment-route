...
const activateUser = async (req, res, next) => {
    ...
    await createNotifs(
        req.user.id, "User Activated",
        `User ${id} Activated Successfully`, 'account',
        undefined, undefined
    )

    res.status(StatusCodes.OK).json({
        msg: `User ${id} is Active now`,
        activate,
    });
};
...
const suspendUser = async (req, res, next) => {
    ...
    await createNotifs(
        req.user.id, "User Suspended",
        `User ${id} Suspended Successfully`, 'account',
        undefined, undefined
    )

    res.status(StatusCodes.OK).json({
        msg: `User ${id} is Suspended now`,
        suspend,
    });
};
...
const exportUsersCSV = async (req, res, next) => {
    ...
    await createNotifs(
        req.user.id, "Users Exported",
        'Users CSV Exported Successfully', 'account',
        undefined, undefined
    )
...
const exportShipmentsCSV = async (req, res, next) => {
    ...
    await createNotifs(
        req.user.id, "Shipments Exported",
        'Shipments CSV Exported Successfully', 'shipments',
        undefined, undefined
    )
    ...
    res.setHeader(
        'Content-Disposition',
        'attachment; filename=shipments-export.csv'
    );
    res.status(StatusCodes.OK).send(csv);
};
...
const exportRoutesCSV = async (req, res, next) => {
    ...
    await createNotifs(
        req.user.id, "Routes Exported",
        'Routes CSV Exported Successfully', 'routes',
        undefined, undefined
    )
